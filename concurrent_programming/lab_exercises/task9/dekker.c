...
void entry_protocol(int nr) {
    int other = 1 - nr; // Indeks drugiego wątku
    // Wejście do protokołu wejścia
    wants[nr] = 1;
    while (wants[other]) {
        if (priority == other) {
            wants[nr] = 0;
            while (priority == other) {
                // Czekaj, az proces drugi zmieni wartość turn
            }
            wants[nr] = 1;
        }
    }
...
void exit_protocol(int nr) {
    priority = 1 - nr; // Ustalamy kolej drugiego wątku
    wants[nr] = 0; // Wycofujemy się z sekcji krytycznej
}
