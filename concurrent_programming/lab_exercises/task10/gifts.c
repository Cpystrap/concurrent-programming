...
int elf_main(int id, struct SharedStorage* s)
{
    ...
    for (int i = 0; i < LOOPS; ++i) {
        ...
        // Lock free slots and critical section.
        sem_wait(&s->sem_free_slots);
        sem_wait(&s->mutex);

        // Add gift to the queue.
        s->buff[s->end] = g;
...
int reindeer_main(int id, struct SharedStorage* s)
{
    ...
    while (true) {
        ...
        if (s->n_gifts_to_deliver <= 0) {
            // Take back changes (so that the remaining reindeers
            // waiting on semaphore will end).
            sem_post(&s->mutex);
            sem_post(&s->sem_filled_slots); 
            break;
        } else {
            ...
            // Take gift from queue.
            int g = s->buff[s->begin];
            s->begin = (s->begin + 1) % BUFFSIZE;

            if (s->n_gifts_to_deliver <= 0) {
                // Take back changes - there will not be any more gifts,
                // so there will not be any more elfs waiting on the semaphore.
                sem_post(&s->mutex);
                sem_post(&s->sem_filled_slots); 
...
int main(void)
{
    ...
    sem_init(&shared_storage->mutex, 1, 1);
    sem_init(&shared_storage->sem_free_slots, 1, BUFFSIZE);
    sem_init(&shared_storage->sem_filled_slots, 1, 0);
    ...
    sem_destroy(&shared_storage->mutex);
    sem_destroy(&shared_storage->sem_free_slots);
    sem_destroy(&shared_storage->sem_filled_slots);
